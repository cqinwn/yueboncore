...
namespace Yuebon.Security.Repositories
{
    ...
    public class TaskManagerRepository : BaseRepository<TaskManager>, ITaskManagerRepository, IScopedDependency
    {
        ...
        /// <returns></returns>
        public override  async Task<bool> SetEnabledMarkByWhereAsync(bool bl, string where, long userId)
        {
        ...
        /// <param name="userId">操作用户</param>
        /// <returns></returns>
        public override async Task<bool> DeleteSoftBatchAsync(bool bl, string where, long userId)
        {
            if (HasInjectionData(where))
            {
                Log4NetHelper.Info(string.Format("检测出SQL注入的恶意数据, {0}", where));
