...
namespace Yuebon.Commons.Helpers
{
    ...
    public static class ExpressionExtensions
    {
        ...
        /// <returns></returns>
        public static T Cof_GetICaching<T>(this ICacheService cache, string key, Func<T> GetFun, int timeSpanMin) where T : class
        {
            var obj = cache.GetByKey(key);
            obj = GetFun();
            if (obj == null)
            {
                obj = GetFun();
                cache.Add(key, obj, timeSpanMin);
            }
            return obj as T;
        }
        /// <summary>
        ...
        /// <param name="timeSpanMin">有效期 单位分钟/param>
        /// <returns></returns>
        public static async Task<T> Cof_AsyncGetICaching<T>(this ICacheService cache, string key, Func<Task<T>> GetFun, int timeSpanMin) where T : class
        {
            var obj = cache.GetByKey(key);
            if (obj == null)
            {
