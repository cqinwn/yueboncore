        ...
        private string GetSqlServerSql(bool isDoCount,bool isSql2012=false)
        {
            ...
            else
            {
                ...
                //    minRow, maxRow);
                    ...
                    sql = String.Format("SELECT * FROM {0} WHERE {1} order by {2} offset (({3} - 1) * {4}) rows fetch next {4} rows only;", this.TableOrSqlWrapper, this.strwhere
                        ,this.fieldNameToSort,pageIndex,pageSize);
                }
            }
