  ...
  <div class="app-container">
    <el-form ref="searchformRef" v-show="showSearch" :inline="true" :model="queryParams" class="demo-form-inline">
      ...
      <el-form-item label="日志日期：" prop="CreateTime">
        <el-date-picker v-model="queryParams.CreateTime" type="datetimerange" align="right"
          :default-time="['00:00:00', '23:59:59']" format="YYYY-MM-DD HH:mm:ss" value-format="YYYY-MM-DD HH:mm:ss"
          unlink-panels range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" :shortcuts="shortcuts" />
      </el-form-item>
      <el-form-item>
    ...
    <el-table ref="gridtable" v-loading="tableloading" :data="tableData" stripe highlight-current-row
      style="width: 100%" :default-sort="{ prop: 'CreatorTime', order: 'descending' }"
      ...
      <el-table-column prop="Result" label="操作状态" sortable="custom" width="120">
        <template #default="scope">
          <el-tag :type="scope.row.Result === true ?  'success': 'danger'" disable-transitions>{{
...
<script setup name="SqlLog">
import { getSqlLogListWithPager, deleteSqlLog } from '@/api/security/sqllogservice'
...
function loadTableData() {
  ...
  getSqlLogListWithPager(seachdata).then((res) => {
